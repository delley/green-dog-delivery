...
public class GreenDogDeliveryApplicationTests {
	...
	public void findClientesByNome() throws Exception {
		...
		this.mvc.perform(get(URL2)).andExpect(status().isOk()).andExpect(jsonPath("endereco", equalTo("Sampa")));
	}
	...
	public void findClientesByNomeContaining() throws Exception {
		...
		this.mvc.perform(get(URL3)).andExpect(status().isOk()).andExpect(jsonPath("_embedded.clientes", hasSize(2)));
	}
